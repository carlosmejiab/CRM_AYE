...
(function (o, K) {
    function Ia(a, b) {
        ...
        if (a.date) { if (!a.start) a.start = a.date; delete a.date } a._start = q(a.start = ib(a.start)); a.end = ib(a.end); if (a.end && a.end <= a.start) a.end = null; a._end = a.end ? q(a.end) : null; if (a.allDay === K) a.allDay = b.allDayDefault; if (a.className) { if (typeof a.className == "string") a.className = a.className.split(/\s+/) } else a.className = []
    } function Wa(a, b, f, c) {
        function g(m, e, j, r) {
            ...
            if (U) {
                t(); e = U.find("tr").length; if (N < e) U.find("tr:gt(" + (N - 1) + ")").remove();
                else if (N > e) { m = ""; for (e = e; e < N; e++){ m += "<tr class='fc-week" + e + "'>"; for (E = 0; E < L; E++){ m += "<td class='fc-" + za[B.getDay()] + " " + ca + "-state-default fc-new fc-day" + (e * L + E) + (E == R ? " fc-leftmost" : "") + "'>" + (r ? "<div class='fc-day-number'></div>" : "") + "<div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></td>"; C(B, 1); O && da(B) } m += "</tr>" } U.append(m) } fa(U.find("td.fc-new").removeClass("fc-new"));
                B = q(A.visStart); U.find("td").each(
                    function () { var W = o(this); if (N > 1) B.getMonth() == s ? W.removeClass("fc-other-month") : W.addClass("fc-other-month"); +B == +I ? W.removeClass("fc-not-today").addClass("fc-today").addClass(ca + "-state-highlight") : W.addClass("fc-not-today").removeClass("fc-today").removeClass(ca + "-state-highlight"); W.find("div.fc-day-number").text(B.getDate()); C(B, 1); O && da(B) }); if (N == 1) {
                        B = q(A.visStart); x.find("th").each(function () {
                            ...
                            function () { this.className = this.className.replace(/^fc-\w+(?= )/, "fc-" + za[B.getDay()]); C(B, 1); O && da(B) })
                }
            } else {
...
A.name=c;A.init(a,b);mb(a.addClass("fc-grid"));var aa=new nb(function(m,e){var j,r,s,I=U.find("tr:first td");if(ra)I=o(I.get().reverse());I.each(function(E,B){j=o(B);r=j.offset().left;if(E)s[1]=r;s=[r];e[E]=s});s[1]=r+j.outerWidth();U.find("tr").each(function(E,B){j=o(B);r=j.offset().top;if(E)s[1]=r;s=[r];m[E]=s});s[1]=r+j.outerHeight()}),Z=new ob(aa),$=false,ia=pb(A,Z,X,function(){return true},ma,ga,Q,ha);A.select=function(m,e,j){aa.build();ha();e||(e=q(m));ma(m,C(q(e),1));Q(m,e,j)};A.unselect=ha;
        qb(A, ha); A.dragStart = function (m, e) { Z.start(function (j) { ga(); j && pa(j.row, j.col, j.row, j.col) }, e) }; A.dragStop = function (m, e, j) {
            var r = Z.stop(); ga();
            if (r) { r = X(r); A.trigger("drop", m, r, true, e, j) }
        }
        ...
        var M = f.options, sa = M.isRTL, G, fa = a.length, D, Q, ha, ma, pa, ga = "", X = {}, ca = {}, ua = [], O = []; for (G = 0; G < fa; G++){
            D = a[G]; Q = D.event; ha = "fc-event fc-event-hori "; if (sa) {
                ...
                if (D.isEnd) ha += "fc-corner-left "; ma = D.isEnd ? k(D.end.getDay() - 1) : c; pa = D.isStart ? y(D.start.getDay()) :
                    g
            } else {
                if (D.isStart) ha += "fc-corner-left ";
                if (D.isEnd) ha += "fc-corner-right "; ma = D.isStart ? k(D.start.getDay()) : c; pa = D.isEnd ? y(D.end.getDay() - 1) : g
...
this.eventElementsByID[a._id];var c,g=a.length;for(c=0;c<g;c++)a[c][0]!=b[0]&&a[c][f]()},eventDrop:function(a,b,f,c,g,n,k){var y=this,w=b.allDay,t=b._id;y.moveEvents(y.eventsByID[t],f,c,g);y.trigger("eventDrop",a,b,f,c,g,function(){y.moveEvents(y.eventsByID[t],-f,-c,w);y.rerenderEvents()},n,k);y.eventsChanged=true;y.rerenderEvents(t)},eventResize:function(a,b,f,c,g,n){var k=this,y=b._id;k.elongateEvents(k.eventsByID[y],f,c);k.trigger("eventResize",a,b,f,c,function(){k.elongateEvents(k.eventsByID[y],
    ...
    }, moveEvents: function (a, b, f, c) { f = f || 0; for (var g, n = a.length, k = 0; k < n; k++){ g = a[k]; if (c !== K) g.allDay = c; ba(C(g.start, b, true), f); if (g.end) g.end = ba(C(g.end, b, true), f); Ia(g, this.options) } }, elongateEvents: function (a, b, f) { f = f || 0; for (var c, g = a.length, n = 0; n < g; n++){ c = a[n]; c.end = ba(C(this.eventEnd(c), b, true), f); Ia(c, this.options) } },
